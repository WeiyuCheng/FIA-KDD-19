...
class MF(GenericNeuralNet):
    ...
    def get_test_params(self, test_index):
        ...
        for layer in ['embedding_layer']:
            for var_name in ['embedding_users', 'embedding_items', 'bias_users', 'bias_items']:
                temp_tensor = tf.get_default_graph().get_tensor_by_name("%s/%s:0" % (layer, var_name))
                ...
                elif 'bias_users' in var_name:
                    temp_embedding = tf.nn.embedding_lookup(tf.reshape(temp_tensor, (self.num_users, 1)), [test_u],
                                                            name="test_user_bias")
                    # temp_embedding = temp_tensor[test_u:(test_u + 1)]
                elif 'bias_items' in var_name:
                    temp_embedding = tf.nn.embedding_lookup(tf.reshape(temp_tensor, (self.num_items, 1)),
                                                            [test_i], name="test_item_bias")
                    # temp_embedding = temp_tensor[test_i:(test_i + 1)]
    ...
    def inference(self, input_x):
        ...
        with tf.variable_scope('embedding_layer'):
            embedding_users = variable_with_weight_decay("embedding_users", [self.num_users * self.embedding_size],
                                                         stddev=1.0 / math.sqrt(float(self.embedding_size)),
                                                         wd=self.weight_decay)
            embedding_items = variable_with_weight_decay("embedding_items", [self.num_items * self.embedding_size],
                                                         stddev=1.0 / math.sqrt(float(self.embedding_size)),
                                                         wd=self.weight_decay)
            user_embedding = tf.nn.embedding_lookup(tf.reshape(embedding_users, (self.num_users, self.embedding_size)),
                                                    input_x[:, 0], name="user_embedding")
            item_embedding = tf.nn.embedding_lookup(tf.reshape(embedding_items, (self.num_items, self.embedding_size)),
                                                    input_x[:, 1], name="item_embedding")

            bias_users = variable("bias_users", [self.num_users], tf.constant_initializer(0.0))
            bias_items = variable("bias_items", [self.num_items], tf.constant_initializer(0.0))
            user_bias = tf.nn.embedding_lookup(tf.reshape(bias_users, (self.num_users, 1)), input_x[:, 0],
                                               name="user_bias")
            item_bias = tf.nn.embedding_lookup(tf.reshape(bias_items, (self.num_items, 1)), input_x[:, 1],
                                               name="item_bias")
            global_bias = variable("global_bias", [1], tf.constant_initializer(0.0))

        with tf.variable_scope('interaction_layer'):
            rating = tf.squeeze(tf.reduce_sum(user_embedding * item_embedding, axis=1,
                                              keep_dims=True) + user_bias + item_bias + global_bias,
                                name="rating_squeeze")
    ...
    def get_test_grad(self, grad_total_loss_op):
        ...
        test_grad.append(
            grad_total_loss_op[2][self.test_u:(1 + self.test_u)])
        test_grad.append(
            grad_total_loss_op[3][self.test_i:(1 + self.test_i)])
        return test_grad
...
#        np.save("output_exp/RQ3-[%s]-train_indices_of_test_case" % (test_indices[0]), self.train_indices_of_test_case)
        ...
        if train_idx is None:
            ...
            for counter, idx_to_remove in enumerate(self.train_indices_of_test_case):
                ...
                # train_grad_loss_val = self.sess.run(self.grad_loss_r_test, feed_dict=single_train_feed_dict)
                predicted_loss_diffs[counter] = np.dot(np.concatenate(inverse_hvp),
